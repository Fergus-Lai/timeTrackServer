...
createConnection().then(async connection => {
    ...
    // Get All Users
    app.get('/allusers/:api', async function(req:Request, res:Response) {
    ...
    app.get("/category/:api/:id", async function(req:Request,res:Response) {
        if (!apiCheck(req.params.api)) {
            return res.sendStatus(403)
        }
    ...
    app.use(function(req:Request, res:Response) {
        res.status(405).send("Invalid Route")
    })

    // start express server
    app.listen(process.env.port)

    console.log(`Express server has started on port ${process.env.port}. Open http://localhost:8080/`)
